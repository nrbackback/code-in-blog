...
func (msg *messageV1) Version() snmpgo.SNMPVersion {
	return msg.version
}

func (msg *messageV1) Pdu() snmpgo.Pdu {
	return msg.pdu
}
...
func (msg *messageV1) String() string {
	return fmt.Sprintf(
		`{"Version": "%s", "Community": "%s", "snmpgo.Pdu": %s}`,
...
func (msg *messageV3) String() string {
	return fmt.Sprintf(
		`{"Version": "%s", "GlobalData": %s, "SecurityParameter": %s, "snmpgo.Pdu": %s}`,
		msg.version, msg.globalDataV3.String(), msg.securityParameterV3.String(),
		msg.pdu.String())
}

func newMessage(ver snmpgo.SNMPVersion) (msg message) {
	switch ver {
	case snmpgo.V1, snmpgo.V2c:
		msg = newMessageWithPdu(ver, &snmpgo.PduV1{})
	case snmpgo.V3:
		msg = newMessageWithPdu(ver, &snmpgo.ScopedPdu{})
	}
	return
}

func newMessageWithPdu(ver snmpgo.SNMPVersion, pdu snmpgo.Pdu) (msg message) {
	m := messageV1{
		version: ver,
		pdu:     pdu,
	}
